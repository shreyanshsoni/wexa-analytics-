...
def process_event(self: object, event_data: dict[str, Any], org_id: str) -> dict[str, Any]:
    logger.info("processing_event", org_id=org_id, event_name=event_data.get("event_name"))
    return {"status": "processed", "org_id": org_id}


@celery_app.task(bind=True, max_retries=3, default_retry_delay=60)  # type: ignore
def process_batch_events(self: object, events: list[Any], org_id: str) -> dict[str, Any]:
    logger.info("processing_batch", org_id=org_id, count=len(events))
    return {"status": "processed", "count": len(events)}


@celery_app.task(bind=True, max_retries=3, default_retry_delay=120)  # type: ignore
def process_csv_upload(self: object, file_path: str, org_id: str) -> dict[str, Any]:
